...
namespace SeniorProjectHealthApplication.Views
{
    ...
    public partial class AccountSettings : ContentPage
    {
        ...
        private void LoadUserSettings(int userId)
        {
            ...
            // Input Label
            FirstNameInput.Text = char.ToUpper(user.First_Name[0]) + user.First_Name.Substring(1).ToLower();
            LastNameInput.Text = char.ToUpper(user.Last_Name[0]) + user.Last_Name.Substring(1).ToLower();
            EmailInput.Text = user.Email;
            GenderInput.SelectedItem = user.Gender;
            BirthdateInput.Date = DateTime.Parse(user.Birthdate);
        }
